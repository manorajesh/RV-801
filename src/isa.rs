...
impl Instruction {
    pub fn from(inst: u32) -> Result<Self, String> {
        let inst_type = parse_inst(inst).map_err(|e| format!("Invalid instruction: {}", e))?;
        let decoded_inst = get_inst(inst_type).map_err(|e| format!("Invalid instruction: {}", e))?;
